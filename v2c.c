/* invesigating how to avoid jitter */
#include<stdio.h>
#include<vips/vips.h>
#include<sys/time.h>
#include<sys/stat.h>
...
int main( int argc, char **argv )
{
    ...
    // ok the point of interest is going to come under alot of stress.
    // We want to work off a neat aspect ratio for us.
    int iw = vips_image_get_width(in); 
    int ih = vips_image_get_height(in); 
    float oasp=(float)iw/ih;
    printf("oasp is %4.4f\n"); 
    float wt=(float)iw/XSZ; // "width times"
    float ht=(float)ih/XSZ;
    printf("wt:%4.6f ht:%4.6f\n", wt, ht);
    printf("viewport asprat:%4.6f oringimasprat:%4.6f\n", (float)XSZ/YSZ, (float)iw/ih);
    
    /*
    int sxf=(float)xpt-(float)XSZ/2.;
...
#endif
        sx=(int)(0.25+(float)xpt*ofac-(float)XSZ/2.);
        sy=(int)(0.25+(float)ypt*ofac-(float)YSZ/2.);
