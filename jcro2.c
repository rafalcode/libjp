...
GLOBAL(boolean) jtransform_request_workspace (j_decompress_ptr srcinfo, jpeg_transform_info *info)
{
    ...
    switch (info->transform) {
        case JXFORM_TRANSPOSE: case JXFORM_TRANSVERSE: case JXFORM_ROT_90: case JXFORM_ROT_270:
...
LOCAL(int) parse_switches (j_compress_ptr cinfo, int argc, char **argv, int last_file_arg_seen, boolean for_real)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        if (keymatch(arg, "arithmetic", 1)) {
            ...
            if (! printed_version) { //config.h and version.h only needed for these!
...
int main (int argc, char **argv)
{
    ...
    /* Old to file thang: Must have either -outfile switch or explicit output file name */
    if (outfilename == NULL) {
